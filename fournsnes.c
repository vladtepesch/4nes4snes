...
static void fournsnesUpdate_multitap(void)
{
  int i;
  unsigned char tmp1=0;
  unsigned char tmp2=0;
...
static void fournsnesUpdate_normal(void)
{
    int i;
    unsigned char tmp1=0;
    unsigned char tmp2=0;
...
static void fournsnesUpdate(void)
{
    ...
    }else if (multitap_mode) {
        fournsnesUpdate_multitap();
    } else { // standard mode (not multitap)
        fournsnesUpdate_normal();
    }
}
...
static char getXY(char x, char y){
  return (y<<4) | x;
}
...
static char fournsnesBuildReport(unsigned char *reportBuffer, char id)
{
    ...
    if (fourscore_mode) {
        ...
        {
            reportBuffer[0] = id;
            reportBuffer[1] = getXY( getX(last_read_controller_bytes[idx]), getY(last_read_controller_bytes[idx]));
            reportBuffer[2] = nesReorderButtons(last_read_controller_bytes[idx]);
        }
    ...
    {
        reportBuffer[0] = id;
        reportBuffer[1] = getXY( getX(last_read_controller_bytes[idx*2]), getY(last_read_controller_bytes[idx*2]));

        if (nesMode & (0x01<<idx))
            reportBuffer[2] = nesReorderButtons(last_read_controller_bytes[idx*2]);
        else
            reportBuffer[2] = snesReorderButtons(&last_read_controller_bytes[idx*2]);
    }
...
#include "fournsnesHIDReportdescriptorData.c"
};
